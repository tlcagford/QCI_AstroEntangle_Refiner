...
def get_image_download_link(fig, filename, title="Download"):
    ...
    href = f'<a href="data:image/png;base64,{b64}" download="{filename}" class="download-button">{title}</a>'
    return href
...
def get_array_download_link(data, filename, title="Download"):
    ...
    buf.seek(0)
    b64 = base64.b64encode(buf.read()).decode()
    href = f'<a href="data:application/octet-stream;base64,{b64}" download="{filename}" class="download-button">{title}</a>'
    return href
...
def get_json_download_link(data, filename, title="Download"):
    ...
    href = f'<a href="data:application/json;base64,{b64}" download="{filename}" class="download-button">{title}</a>'
    return href
...
with tab1:
    ...
    with col2:
        ...
        if astro_image is not None:
            ...
            # Download comparison figure
            comp_filename = f"{base_filename}_comparison.png"
    ...
    if astro_image is not None and radar_data:
        ...
        st.markdown(get_image_download_link(fig, radar_filename, "📡 Download Radar Overlay"), unsafe_allow_html=True)
        plt.close(fig)
...
with tab2:
    ...
    with col1:
        ...
        with col_d1:
            ...
            # Download B-field array
            st.markdown(get_array_download_link(B_mag, f"{base_filename}_bfield.npz", "📊 Download B-Field Array"), unsafe_allow_html=True)
        
        with col_d2:
            # Download QED polarization
...
with tab3:
    ...
    with col1:
        ...
        st.markdown(get_array_download_link(np.array(mixing_prob), f"{base_filename}_mixing.npy", "📊 Download Mixing Array"), unsafe_allow_html=True)
    ...
    if radar_data:
        ...
        if detections:
            ...
            correlation_data = {
                ...
                "entanglement_entropy": entropy[-1],
                "mixing_probability": mixing_prob[-1]
            }
            st.markdown(get_json_download_link(correlation_data, f"{base_filename}_radar_correlation.json", "📡 Download Radar Correlation"), unsafe_allow_html=True)
...
with st.expander("📖 About QCAUS", expanded=False):
    ...
    **Download Features:**
    - All visualizations can be downloaded as PNG
    - Arrays available as NPZ/NPY
    - Metadata as JSON with parameter tracking
    - Smart file naming: `project_target_parameters_timestamp`
    """)
